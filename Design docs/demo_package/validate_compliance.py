...
def main():
    ...
    for t, cols in model["tables"].items():
        ...
        for c in cols:
            ...
            if g == "fk_lookup_jitter":
                ...
                ratio = (df[n] / df[fk].map(lut)).dropna()
                jp = p["jitter_pct"]
    ...
    for v_name, spec in model.get("views", {}).items():
        ...
        joined = {sources[0]}
        for jl in [x.strip() for x in spec.get("join_logic", "").split(";") if x.strip()]:
        ...
        gb = [c.strip() for c in spec["group_by"].split(",") if c.strip()]
        ...
        got = pd.read_csv(f).sort_values(gb).reset_index(drop=True)
