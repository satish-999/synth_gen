...
class Engine:
    ...
    def generate_table(self, name):
        ...
        for c in cols:
            ...
            if c["fk_ref"]:                                     # REFERENCE fk
                ...
                df[cn] = np.round(df[fk].map(lut).astype(float) * jit,
                                  int(p.get("round", 2)))
    ...
    def materialize_view(self, name):
        ...
        for j in [x.strip() for x in spec.get("join_logic", "").split(";") if x.strip()]:
        ...
        if spec.get("filter_logic"):
            df = df.query(spec["filter_logic"])

        gb = [c.strip() for c in spec.get("group_by", "").split(",") if c.strip()]
        ...
        for c in post_cols:
            ...
            out[c["name"]] = np.round(out.eval(inner.group(1)),
                                      int(inner.group(2)))
        self.frames[name] = out[[c["name"] for c in spec["columns"]]]
